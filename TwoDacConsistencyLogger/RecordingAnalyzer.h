...
public:
	...
	/// </summary>
	static RecordingResult AnalyzeRecording(const GeneratedSamples& config, const SystemInfo& sysInfo, const WasapiOutput& output, const WasapiInput& input, float detectionThresholdMultiplier);
...
private:
	...
	static RecordingSingleChannelResult AnalyzeSingleChannel(const GeneratedSamples& config, float* recordedSamples, int recordedSamplesLength, int inputSampleRate, float detectionThresholdMultiplier);
	static void SaveResult(const GeneratedSamples& config, const SystemInfo& sysInfo, int inputSampleRate, RecordingResult result, std::string testRootPath, std::string recordingRootPath, std::string outputDeviceName, std::string inputDeviceName);
