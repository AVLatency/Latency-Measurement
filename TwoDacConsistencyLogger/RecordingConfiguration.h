...
public:
	...
	// - Tick pattern: 0.5 s, 0.6 s, 0.8 s
	// - Pattern is spaced unevenly to prevent echo/other speakers from interfering with detection
	// - Padding at the start allows time for audio device to wake up
	//   ...More may be needed. I know some recievers take multiple seconds before playing back audio
	// - This initial pause will cause drift between devices in ASIO4ALL, so it must be kept consistent
