...
#include "stb_image.h" // Create the implementation now that STB_IMAGE_IMPLEMENTATION has been defined.
...
bool LoadTextureFromData(unsigned char* res_data, DWORD res_size, ID3D11ShaderResourceView** out_srv, int* out_width, int* out_height, ID3D11Device* g_pd3dDevice)
{
    ...
    int image_width = 0;
    int image_height = 0;
    unsigned char* image_data = stbi_load_from_memory(res_data, res_size, &image_width, &image_height, NULL, 4);
    if (image_data == NULL)
        return false;
