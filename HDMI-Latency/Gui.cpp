...
bool Gui::DoGui()
{
    ...
    if (ImGui::BeginTable("MainViewTopLevelTable", 2, ImGuiTableFlags_Borders, ImVec2(1234 * DpiScale, 0)))
    {
        ...
        case MeasurementToolGuiState::FinishingAdjustVolume:
        {
            ...
            {
                GuiHelper::AdjustVolumeDisplay(adjustVolumeManager->LeftVolumeAnalysis, DpiScale, "Input: Left Channel (HDMI Audio Extractor)");
                GuiHelper::AdjustVolumeDisplay(adjustVolumeManager->RightVolumeAnalysis, DpiScale, "Input: Right Channel (DUT)");
...
void Gui::StartTest()
{
    ...
    if (testManager == nullptr)
    {
        ...
        testManager = new TestManager(outputAudioEndpoints[outputDeviceIndex], inputAudioEndpoints[inputDeviceIndex], selectedFormats, fileString, APP_FOLDER, (IResultsWriter&)ResultsWriter::Writer, HdmiOutputOffsetProfiles::CurrentProfile(), &DacLatencyProfiles::None);
    }
}
